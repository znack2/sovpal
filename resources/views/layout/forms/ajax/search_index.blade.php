<form class="text-center" action="{{ route(Request::route()->getName(),['type'=>Request::input('type')]) }}" method="GET" accept-charset="UTF-8">
  {!! csrf_field() !!}
        ...
        <input autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" id="field" name="field" type="hidden"
        value="{{ Request::input('type') == 'owner' || Request::input('type') == 'profi' ? trans('sovpal.forms.first_name')  : trans('sovpal.forms.title')}}">
</form>
